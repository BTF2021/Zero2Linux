...
public partial class Incarcare : Node2D
{
	...
	{	_data = (DefaultData)GetNode("/root/DefaultData");
		...
		Godot.Collections.Array<string> splash = new Godot.Collections.Array<string>(){		//Mesajele de mai jos. Se alege unul la intamplare
			...
			"Ai grija cand folosesti comand [i]rm[/i]",
			"Doar pentru ca folosesti Linux nu inseamna ca nu o ai virusi",
			"E GNU/Linux",
			...
			"[i]Poate sa ruleze MS Paint?[/i]",
			"[i]Pur si simplu functioneaza[/i]\n\t-Absolut nimeni",
			"[i]Nu uita sa respiri. Foarte important[/i]",
			"[i]Hello, your computer has virus :)[/i]",
